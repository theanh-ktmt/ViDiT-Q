...
class IDDPM(SpacedDiffusion):
    ...
    def sample(
        ...
        n = len(prompts)
        z = (
            torch.randn(n, *z_size, device=device)
            if init_noise is None
            else init_noise.to(device)
        )
        z = torch.cat([z, z], 0)
        ...
        if sampler_type == "ddim":
            ...
            raise NotImplementedError(
                f"{sampler_type} is not a supported sampler type!"
            )
...
def forward_with_cfg(
    model, x, timestep, y, cfg_scale, return_trajectory=False, **kwargs
):
    ...
    CFG_SPLIT = (
        model.cfg_split if hasattr(model, "cfg_split") else False
    )  # DIRTY: read the cfg_split cfg from model
    if CFG_SPLIT:
        # DEBUG_ONLY
    ...
    # NOTE: Missing file "k_for_each_timestep.pth"
