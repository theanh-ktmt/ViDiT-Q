...
from qdiff.models.quant_block import (
    BaseQuantBlock,
...
from qdiff.models.stdit_quant_layer import (
    QuantCrossAttnLinear,
    QuantSpatialAttnLinear,
    QuantTemporalAttnLinear,
)
from qdiff.quantizer.base_quantizer import (
    ActQuantizer,
    BaseQuantizer,
    StraightThrough,
    WeightQuantizer,
...
class QuantModel(nn.Module):
        ...
        for name, child_module in module.named_children():
            ...
            if isinstance(child_module, (nn.Conv2d, nn.Conv1d, nn.Linear)):
                ...
                if self.model_type == "opensora":
                    assert isinstance(
                        tmp_module, nn.Linear
                    )  # only linear layers to quantizr in stdit model
                if ".attn." in full_name:
                    ...
                    setattr(
                        module,
                        name,
                        QuantLayer(child_module, weight_quant_params, act_quant_params),
                    )
    ...
    def set_quant_state(self, weight_quant: bool = False, act_quant: bool = False):
        ...
        if hasattr(self, "fp_layer_list"):
            self.set_layer_quant(
                model=self,
                module_name_list=self.fp_layer_list,
    ...
    def set_module_name_for_quantizer(self, module, prefix=""):
        ...
        for name_, module_ in module.named_children():
            ...
            if isinstance(module_, BaseQuantizer):  # end with quantizer module
                ...
                self.set_module_name_for_quantizer(
                    module=module_, prefix=full_name + "."
                )
            ...
            if isinstance(module_, BaseQuantizer):  # end with quantizer module
                if module_.timestep_wise:
                    ...
                    module_.delta_list = torch.repeat_interleave(
                        module_.delta_list[:, choose_idx, :],
                        1000 // len(choose_idx),
                        dim=1,
                    )
                    module_.zero_point_list = torch.repeat_interleave(
                        module_.zero_point_list[:, choose_idx, :],
                        1000 // len(choose_idx),
                        dim=1,
                    )
            else:
    ...
    def set_quant_params_dict(
        self, quant_params_dict, module=None, load_buffer_only=True, dtype=torch.float32
    ):
        ...
        for name, module_ in module.named_children():
            ...
            if isinstance(module_, quantizer_type):
                ...
                if load_buffer_only:
                    assert (
                        len(quant_params_dict[module_.module_name][1]) == 0
                    )  # parameters() has no element
                    for name, quant_params in quant_params_dict[module_.module_name][
                        0
                    ].items():  # use module_name to index the dict
                        setattr(
                            module_,
                            name,
                            quant_params.to(dtype)
                ...
                self.set_quant_params_dict(
                    quant_params_dict=quant_params_dict, module=module_
                )
                    ...
                    if opt_d[opt_target] is not None:
                        for param_type in opt_d[opt_target]:
                            ...
                            if module_.module_name.split(".")[0] in self.fp_layer_list:
                                ...
                                module_.register_parameter(
                                    param_type, nn.Parameter(buffer_)
                                )
                else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        self.set_timestep_id_for_quantlayer(
            t[0].item()
        )  # used for timestep-wise smooth quant alpha
    ...
    def set_layer_smooth_quant(
        self,
        model,
        module_name_list,
        smooth_quant,
        smooth_quant_running_stat,
        prefix="",
    ):
        for name, module in model.named_children():
            ...
            if isinstance(module, QuantLayer):
                for module_name in module_name_list:
                    if pattern_in(full_name, module_name) or pattern_in(
                        full_name, "model." + module_name
                    ):
                        ...
                        logger.info(
                            f"{full_name}: smooth_quant={smooth_quant} | smooth_quant_running_stat={smooth_quant_running_stat}"
                        )
            else:
                self.set_layer_smooth_quant(
                    model=module,
                    module_name_list=module_name_list,
                    smooth_quant=smooth_quant,
                    smooth_quant_running_stat=smooth_quant_running_stat,
                    prefix=full_name + ".",
                )
                ...
                if isinstance(module, QuantLayer):
                    for module_class in group_list:
                        if module_class == "attn":
                            if (
                                module_class in full_name
                                and not "cross_attn" in full_name
                                and not "attn_temp" in full_name
                            ):
                                if all(
                                    element not in full_name for element in group_ignore
                                ):
                                    ...
                                    torch.cuda.empty_cache()
                                    logger.info(
                                        f"{full_name}: weight_quant={weight_quant}, act_quant={act_quant}"
                                    )
                        else:
                ...
                if full_name in bit_config.keys():
                    if bit_type == "weight":
                        ...
                        if hasattr(module, "weight_quantizer_0"):
                            module.weight_quantizer_0.bitwidth_refactor(
                                bit_config[full_name]
                            )
                        ...
                        if hasattr(module, "act_quantizer_0"):
                            module.act_quantizer_0.bitwidth_refactor(
                                bit_config[full_name]
                            )
