    ...
    def forward(self, input: torch.Tensor, scale: float = 1.0, split: int = 0):
        ...
        if self.smooth_quant:
            ...
            if self.channel_wise_scale_type == "dynamic":
                channel_wise_scale = input.abs().max(dim=-2)[0].pow(alpha).mean(
                    dim=0, keepdim=True
                ...
                if self.smooth_quant_running_stat:
                    ...
                    if self.act_quantizer.act_scale is None:
                        self.act_quantizer.act_scale = torch.zeros(
                            [self.timerange_num, *cur_act_scale.shape]
                        ).to(input)
                    if self.act_quantizer.act_scale[cur_timerange_id].abs().mean() == 0:
                        self.act_quantizer.act_scale[cur_timerange_id] = cur_act_scale
                    else:
                        self.act_quantizer.act_scale[
                            cur_timerange_id
                        ] = self.act_quantizer.act_scale[
                            cur_timerange_id
                        ] * self.smooth_quant_momentum + cur_act_scale * (
                    ...
                    if (self.act_quantizer.act_scale[cur_timerange_id] == 0).sum() != 0:
                        ...
                        logging.info(
                            "act_scale containing zeros, replacing with {}".format(eps)
                        )

                channel_wise_scale = self.act_quantizer.act_scale[cur_timerange_id].pow(
                    alpha
                ) / self.weight.abs().max(dim=0)[0].pow(1 - alpha)
            else:
            ...
            if not hasattr(self, "timerange"):
                ...
                cur_timerange_id = find_interval(self.timerange, self.cur_timestep_id)
            if (
                getattr(self, "smooth_quant_running_stat", False)
                and "momentum" in self.channel_wise_scale_type
            ):
                cur_act_scale = input.abs().max(dim=-2)[0].mean(dim=0, keepdim=True)
                if self.act_quantizer.act_scale is None:
                    self.act_quantizer.act_scale = torch.zeros(
        ...
        if self.weight_quant:
            if self.smooth_quant:
                ...
                if (
                    self.weight_quantizer.timestep_wise is None
                ):  # reinit the weight_quantizer
        ...
        if not self.disable_act_quant and self.act_quant:
            ...
            if layer_type == "q":
                ...
                if not self.act_quant_params.get("dynamic", False):
                    if (
                        self.act_quant_params.per_group is False
                    ):  # no need to reshape for tensor-wise quant
                        input = self.act_quantizer(input)
                    else:
