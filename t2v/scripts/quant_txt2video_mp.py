...
def timestep_set_quant(
    ...
    for i in range(len(quant_time_list)):
        ...
        qnn.quant_start_t = quant_time_list[split_timestep - i - 1] + (
            n_step // split_timestep - 1
        )
        qnn.quant_end_t = quant_time_list[split_timestep - i - 1]

        save_dir = os.path.join(
            opt.save_dir, f"t_{n_step}/{qnn.quant_start_t}_{qnn.quant_end_t}"
        )
        os.makedirs(save_dir, exist_ok=True)
...
def main():
    ...
    # It is not required for STDiT
    # calib_added_cond = {}

    # NOTE: Remove this temporarily
    # with torch.no_grad():
    ...
    if opt.part_fp:
        ...
        qnn.set_quant_state(
            use_weight_quant, use_act_quant
    ...
    if aq_params.smooth_quant.enable:
        ...
        qnn.set_smooth_quant(
            smooth_quant=True, smooth_quant_running_stat=False
        )  # Now we use fp16 to save the statistic of activation
        qnn.set_layer_smooth_quant(
            model=qnn,
            module_name_list=fp_layer_list,
            smooth_quant=False,
            smooth_quant_running_stat=False,
        )
